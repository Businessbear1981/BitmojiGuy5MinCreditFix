...
from api.mail_service import send_all_letters as lob_send_all, verify_webhook_signature as lob_verify_sig
from api.email_sender import send_letters_email
...
def api_payment_success(sid: str = "", response: Response = None, db: Session = Depends(get_db_dependency)):
    if sid:
        ...
        if sub:
            ...
            _store(sid, sub, db)
    if sub:
        _post_payment(sub, sid)
    return RedirectResponse(f"{FRONTEND_URL}/gate", status_code=302)


def _post_payment(sub: dict, sid: str):
    """Send certified mail via Lob and email PDF to client after payment."""
    letters = sub.get("letters", [])
    name = sub.get("name", "")
    email = sub.get("email", "")
    if letters and not sub.get("mail_sent"):
        results = lob_send_all(name, sub.get("address", ""), letters, sid)
        if results:
            sub["mail_tracking"] = results
            sub["mail_sent"] = True
    if email and not sub.get("email_sent"):
        sent = send_letters_email(email, name, sid, None)
        if sent:
            sub["email_sent"] = True
...
def admin_approve_payment(body: AdminApproveRequest, request: Request, db: Session = Depends(get_db_dependency)):
    _require_admin(request)
    target_sid = body.session_id
