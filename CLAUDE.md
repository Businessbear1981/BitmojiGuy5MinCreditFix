...
- **Temp uploads**: Written to OS temp dir (`tempfile.mkdtemp`), removed after parsing. No persistent disk required on Railway.
...
Environment variables required in Railway dashboard → Variables:
- `DATABASE_URL` — `postgresql://postgres:<password>@db.<project>.supabase.co:5432/postgres`
- `SECRET_KEY` — generate a strong random string
- `ADMIN_KEY` — admin dashboard password (rotate from default before launch)
...
- `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS`, `FROM_EMAIL` — confirmation emails (optional, gracefully skipped if not set)
- `CLICK2MAIL_API_KEY` — white-glove certified mail add-on (optional)

Generate a public domain: Railway → service → Settings → Networking → Generate Domain.

## Vercel Deployment (frontend)
Environment variables in Vercel dashboard → Settings → Environment Variables:
- `NEXT_PUBLIC_FLASK_URL` — the Railway public domain (e.g. `https://xxx.railway.app`)
- `NEXT_PUBLIC_SUPABASE_URL` — `https://xapuuymbsmkpsezuccly.supabase.co`
