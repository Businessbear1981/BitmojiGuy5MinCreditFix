...
import { createCheckout, manualPay, queueForRelease, setApiSessionId } from '@/lib/api'
...
export default function StairwayPage() {
  ...
  // so the customer can retry payment after a refresh.
  useEffect(() => {
  ...
  async function handleManual(method: 'cashapp' | 'chime') {
    ...
    try {
      ...
      if (data.checkout_url) {
        // Cash App / Chime route through the same Stripe Checkout page
        window.location.href = data.checkout_url
