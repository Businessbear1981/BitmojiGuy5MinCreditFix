...
10. **The scoring ledger still never records.** `init_outcomes()` is never called and the three write fns have no callers, so `outcomes.db` stays empty and every score falls back to hardcoded `PRIORS`. **Any confidence number the engine reports today is `HAND_SET`, not `MARKET_DERIVED`.** Partly addressed in `6be9923`: `scoring.py` no longer swallows the failure with a bare `except Exception: pass` — it catches `sqlite3.Error` and reports once per process that the ledger is unreadable, so the condition is visible instead of silent. The remaining fix is to call `init_outcomes()` at startup (as `provenance.init_audit()` already is, `main.py:91`) and wire the three write fns. `verified` 2026-08-31
...
- Next, in order (Iron Sharpens Iron: one fault, fixed, re-run from the top — not a backlog to work through): (1) call `init_outcomes()` at startup and wire the three write fns — gap 10 makes every confidence number `HAND_SET`; (2) call `attach_findings` so student-loan findings reach letters — gap 11; (3) wire `ReliefPanel` into `/koi-pond` + add the `student_loan` bucket to `lib/types.ts`; (4) refresh the forgiveness directory for RAP/post-SAVE — gap 12; (5) write the relief ADR; (6) browser walk before asking Kevin to merge.
